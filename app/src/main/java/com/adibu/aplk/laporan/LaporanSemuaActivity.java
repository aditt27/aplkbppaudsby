...
public class LaporanSemuaActivity extends AppCompatActivity {
    ...
    private LaporanSemuaRVAdapter mLaporanSemuaRVAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLaporanSemuaRVAdapter = new LaporanSemuaRVAdapter(mListLaporan);
        recyclerView.setAdapter(mLaporanSemuaRVAdapter);
        ...
        mSwipeRefresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                if(Helper.isInternetConnected(getApplicationContext())) {
                    ...
                    Toast.makeText(LaporanSemuaActivity.this, getString(R.string.nointernet), Toast.LENGTH_SHORT).show();
        ...
        JsonObjectRequest objectRequest = new JsonObjectRequest(Request.Method.GET, URL, null, new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                ...
                try {
                    ...
                    JSONArray jsonListSurat = response.getJSONArray("laporanLain");
                    ...
                    for(int i=0;i<jsonListSurat.length();i++) {
                        mListLaporan.add(new LaporanSuratModel(
                                ...
                                jsonListSurat.getJSONObject(i).getString("nama")
                        ));
                    }

                    //update adapter setelah masukin ke arraylist informasi
                    mLaporanSemuaRVAdapter.notifyDataSetChanged();
    ...
    private class LaporanSemuaRVAdapter extends RecyclerView.Adapter<LaporanSemuaRVAdapter.ViewHolder> {

        private ArrayList<LaporanSuratModel> listLaporan;

        public LaporanSemuaRVAdapter(ArrayList<LaporanSuratModel> listLaporan) {
            this.listLaporan = listLaporan;
        }

        @NonNull
        @Override
        public LaporanSemuaRVAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_laporan, parent, false);
            LaporanSemuaRVAdapter.ViewHolder viewHolder = new LaporanSemuaRVAdapter.ViewHolder(view);
            return viewHolder;
        }

        @Override
        public void onBindViewHolder(@NonNull final LaporanSemuaRVAdapter.ViewHolder holder, int position) {
            holder.judul.setText(listLaporan.get(position).getNama());
            holder.kiri.setText(listLaporan.get(position).getNoSurat());
            holder.kanan.setText(listLaporan.get(position).getWaktu());
            holder.itemLayout.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent i = new Intent(LaporanSemuaActivity.this, LaporanDetailActivity.class);
                    ...
                    i.putExtra("nama", listLaporan.get(holder.getAdapterPosition()).getNama());
                    i.putExtra("noLaporan", listLaporan.get(holder.getAdapterPosition()).getNoLaporan());
                    startActivity(i);
                }
            });
        }
